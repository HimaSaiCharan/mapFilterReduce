...
const isSizeGreaterThan5 = function (element) {
  return element.length > 5;
};

// words with more than 5 letters ["apple", "banana", "kiwi", "grape"] => ["banana"]
const filterLongWords = function (words) {
  return words.filter(isSizeGreaterThan5);
};
...
const doesBookHaveMorePages = function (book) {
  return book.pages > 200;
};

// books with more than 200 pages [{title: "Book 1", pages: 150}, {title: "Book 2", pages: 250}] => [{title: "Book 2", pages: 250}]
const filterLongBooks = function (books) {
  return books.filter(doesBookHaveMorePages);
};

const isIncompleteProfile = function (user) {
...
const hasHighGrades = function (student) {
  return student.grade > 80;
};

// students with grades above 80 [{name: "John", grade: 75}, {name: "Jane", grade: 85}] => [{name: "Jane", grade: 85}]
const filterHighGrades = function (students) {
  return students.filter(hasHighGrades);
};
...
const getAveragePrice = function (products) {
...
const filterStudentsWithAllSubjectsPassed = function (students) {
  return students.filter(isStudentPassed);
};
...
const filterBirthdaysThisMonth = function (people) {
  ...
  return people.filter(function (person) {
    return person.birthDate.slice(5, 7) === thisMonth;
  });
};

const getAverageAmount = function (orders) {
...
const filterHighValueOrders = function (orders) {
  const averageAmount = getAverageAmount(orders);

  return orders.filter(function (order) {
    return order.amount > averageAmount;
  });
};

const getAverageRating = function (books) {
  const totalBookRating = books.reduce(function (total, book) {
    return total + book.rating;
  }, 0);

  return totalBookRating / books.length;
};
...
const getAverageSalary = function (employees) {
...
const getAverageLikes = function (posts) {
  const totalLikes = posts.reduce(function (total, post) {
...
const filterPopularPosts = function (posts) {
  const averageLikes = getAverageLikes(posts);

  return posts.filter(function (post) {
    return post.likes > averageLikes;
  });
};

const getAveragePostCount = function (posts) {
  const totalLikes = posts.reduce(function (total, post) {
    return total + post.postCount;
  }, 0);
...
const filterActiveUsersByPostCount = function (users) {
  const averagePostCount = getAveragePostCount(users);

  return users.filter(function (user) {
    return user.postCount > averagePostCount;
  });
};

// filter people older than a certain age [{name: "Alice", age: 25}, {name: "Bob", age: 30}, {name: "Charlie", age: 22}] => [{name: "Bob", age: 30}]
const filterByAge = function (people, threshold) {
  return people.filter(function (person) {
...
const filterByPrice = function (products, threshold) {
  return products.filter(function (product) {
...
const filterOrdersByBalanceAndTotal = function (orders, balanceThreshold, totalThreshold) {
  return orders.filter(function (order) {
    return order.customer.balance >= order.order.total;
  });
};
...
const filterCoursesByInstructorAndRating = function (courses, instructorName, ratingThreshold) {
  return courses.filter(function (eachCourse) {
    return eachCourse.course.rating > ratingThreshold && eachCourse.instructor.name === instructorName;
  });
};
...
const filterCandidatesByExperienceAndSkills = function (candidates, experienceThreshold, requiredSkills) {
  return canditates.filter(function (candidate) {
    return candidate.experience > experienceThreshold && candidate.includes(candidate.skills.language);
  });
};
...
const filterCustomersByRegionAndStatus = function (customers, region, status) {
  return customers.filter(function (customer) {
    return customer.region === region && status === customer.membership.status;
  });
};
...
const filterTasksByRoleAndPriority = function (tasks, role, priorityLevel) {
  return tasks.filter(function (task) {
    return task.assignee.role === role && task.task.priority === priorityLevel;
  });
};
